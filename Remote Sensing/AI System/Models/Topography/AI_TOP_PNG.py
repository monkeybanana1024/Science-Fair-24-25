...
def main():
    ...
    for subdir in subdirs:
        png_path = os.path.join(root_dir, subdir, "normalized_topography.png")
        susc_path = os.path.join(root_dir, subdir, "susc.txt")

        if os.path.exists(png_path) and os.path.exists(susc_path):
            img = load_png_image(png_path, target_shape=(128, 128))
            if img is not None:
                try:
    ...
    model = cnn_model(input_shape=(128, 128, 3))  # RGB input shape

    log_dir = os.path.join("AI System/Topography/logs/", "AI_NORM_PNG-" + datetime.now().strftime("%Y%m%d-%H%M%S"))
