...
# Function to load and resize single-band GeoTIFF (aspect.tif)
def load_single_band_geotiff(file_path, target_shape=(128, 128)):
    try:
        with rasterio.open(file_path) as src:
            # Read only the first band (single-channel)
            img = src.read(1, out_shape=(1, target_shape[0], target_shape[1]), resampling=Resampling.bilinear)
            img = np.transpose(img, (1, 2, 0))  # Shape (height, width, 1)
            return img
...
def main():
    ...
    for subdir in subdirs:
        ...
        if os.path.exists(aspect_path) and os.path.exists(susc_path):
            img = load_single_band_geotiff(aspect_path, target_shape=(128, 128))
            if img is not None:
                try:
    ...
    # Normalize images
    scaler = MinMaxScaler()
    for i in range(images.shape[-1]):  # Normalize the single channel
    ...
    model = cnn_model(input_shape=(128, 128, 1))  # Single channel input shape

    # TensorBoard log directory
    log_dir = os.path.join("AI System/Slope/logs/", "AI_ASP_TIF-" + datetime.now().strftime("%Y%m%d-%H%M%S"))
