...
def get_ndvi(image):
    """Calculate NDVI from Sentinel-2 Surface Reflectance Harmonized imagery."""
    ndvi = image.normalizedDifference(['B8', 'B4']).rename('NDVI')  # B8 is Near Infrared, B4 is Red
...
def apply_color_map(ndvi_array):
    ...
    # Example of how you might apply a color map (this function can be expanded further as needed)
    # Normalize NDVI values to range [0, 255] and apply a color map (example using a simple color gradient)
    ndvi_normalized = ((ndvi_array - (-1)) * (255 / 2)).clip(0, 255).astype(np.uint8)  # Example color map scaling
